...
class EnhancedPerformanceCalculator:
    ...
    def calculate_portfolio_performance(self, system, timeout_seconds=300):
        ...
        try:
            ...
            try:
                ...
                if percent_portfolio is not None:
                    ...
                    if len(percent_curve) > 1:
                        ...
                        print(
                            f"âœ… Using built-in percentage returns: {len(daily_returns)} points"
                        )
            except Exception as e:
                ...
                try:
                    ...
                    if len(pnl_curve) >= 2:
                        ...
                        if len(pnl_clean) >= 2:
                            ...
                            print(
                                f"ðŸ“Š P&L conversion: {len(daily_returns)} returns calculated"
                            )
                except Exception as e:
            ...
            if (
                hasattr(self, "warm_up_days")
                and self.warm_up_days
                and len(daily_returns) > self.warm_up_days
            ):
            ...
            print(
                f"ðŸ” DEBUG - Returns range: {daily_returns.min():.6f} to {daily_returns.max():.6f}"
            )
            ...
            sharpe_ratio = (mean_daily_return / daily_std) * np.sqrt(
            ...
            metrics = self._calculate_comprehensive_metrics_fixed(
                daily_returns, cumulative_returns, sharpe_ratio
            )
    ...
    def _calculate_comprehensive_metrics_fixed(
        self, returns, cumulative_returns, sharpe_ratio
    ):
        ...
        profit_factor = (
            ...
            if avg_loss != 0 and len(negative_returns) > 0
            else float("inf")
        )
        ...
        avg_return_to_drawdown_ratio = (
            abs(annual_return / avg_drawdown) if avg_drawdown != 0 else float("inf")
        )
    ...
    def analyze_etf_rule_performance_enhanced(self, system):
        ...
        try:
            ...
            print(
                f"ðŸ“ˆ Analyzing rules across {len(sample_instruments)} sample instruments..."
            )
                ...
                try:
                    ...
                    for instrument in sample_instruments:
                        try:
                            ...
                            forecast = system.rules.get_raw_forecast(
                            ...
                            if forecast is not None and len(forecast) > 0:
                                ...
                                if prices is not None and len(prices) > 1:
                                    ...
                                    if len(aligned_data) > 50:
                                        rule_forecasts.extend(
                                            aligned_data["forecast"].values
                                        )
                                        rule_returns.extend(
                            ...
                            print(
                                f"âš ï¸ Could not analyze {rule_name} for {instrument}: {e}"
                            )
                            continue
                    ...
                    if len(rule_forecasts) > 100:
                        ...
                        if forecast_std > 1e-10:
                            ...
                            if (
                                len(rule_specific_returns) > 50
                                and np.std(rule_specific_returns) > 1e-10
                            ):
                                # Calculate performance metrics
                                mean_return = np.mean(rule_specific_returns)
                                vol = np.std(rule_specific_returns)
                                sharpe = (
                                    (mean_return / vol) * np.sqrt(252) if vol > 0 else 0
                                )
                                annual_return = mean_return * 252

                                # Hit rate
                                positive_returns = rule_specific_returns[
                                    rule_specific_returns > 0
                                ]
                                hit_rate = len(positive_returns) / len(
                                    rule_specific_returns
                                )
                        ...
                        print(
                            f"âš ï¸ {rule_name}: Insufficient data points ({len(rule_forecasts)})"
                        )
                ...
                if portfolio_curve is not None:
                    ...
                    if len(portfolio_returns) > 0 and portfolio_returns.std() > 0:
                        ...
                        for rule_name in rules.keys():
                            ...
                            estimated_return = (
                                portfolio_returns.mean() * 252 * multiplier
                            )
                ...
                try:
                    ...
                    if prices is not None and len(prices) > 50:
                        ...
                        if len(etf_returns) > 0 and etf_returns.std() > 0:
                            etf_sharpe = (
                                etf_returns.mean() / etf_returns.std()
                            ) * np.sqrt(252)
                            ...
                            print(
                                f"âœ… {instrument}: Sharpe {etf_sharpe:.3f}, Return {etf_annual_return:.1%}"
                            )
                        else:
    ...
    def get_etf_statistics(self, system):
        ...
        try:
            ...
            etf_stats["rules_count"] = (
                len(system.rules.trading_rules()) if hasattr(system, "rules") else 0
            )
    ...
    def _display_comprehensive_report(self, report):
        ...
        if "rule_performance" in report and report["rule_performance"]:
            ...
            for rule_name, perf in report["rule_performance"].items():
                method_note = (
                    f" ({perf.get('method', 'unknown')})" if "method" in perf else ""
                )
                print(
        ...
        if "instrument_performance" in report and report["instrument_performance"]:
            ...
            sorted_instruments = sorted(
                report["instrument_performance"].items(),
                key=lambda x: x[1].get("sharpe", 0),
                reverse=True,
            )
            for instrument, perf in sorted_instruments[:5]:
                print(
                    f" â€¢ {instrument}: Sharpe {perf.get('sharpe', 0):.3f}, Return {perf.get('annual_return', 0):.1%}"
                )
