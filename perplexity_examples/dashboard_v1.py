...
class SimpleETFDashboard:
        ...
        try:
            ...
            for instrument in instruments:
                try:
                    ...
                    vol_data = self._get_detailed_volatility_info(
                        system, instrument, vol_target
                    )
                    volatility_details[instrument] = vol_data

                    # Calculate cash weight using Carver's formula
                    if (
                        vol_data["volatility_scalar"]
                        and vol_data["volatility_scalar"] > 0
                    ):
            ...
            print(
                f"📊 Deployment Ratio: {total_cash_deployed / system_portfolio_value:.2f}x"
            )
            print(f"\n📈 Position Details:")
            ...
            for instrument, weight in normalized_cash_weights.items():
                if weight > 0.01:  # Only print significant positions
                    ...
                    print(
                        f"  {instrument}: {weight:.2%} (${cash_pos:,.0f}) "
    ...
    def compare_risk_vs_cash_weights(self, system):
        ...
        vol_target = (
            getattr(system.config, "percentage_vol_target", 12.0) / 100
        )  # Convert to decimal
        ...
        print(
            "💡 Formula: Vol Scalar = Vol Target ÷ Annual Vol, Cash Weight = Risk Weight × Vol Scalar"
        )
    ...
    def get_volatility_scalar_safe(self, system, instrument):
        ...
        try:
            ...
            if (
                subsystem_pos is not None
                and combined_forecast is not None
                and len(subsystem_pos) > 0
                and len(combined_forecast) > 0
            ):
        ...
        try:
            ...
            if prices is not None and len(prices) >= 35:
                ...
                if len(returns) >= 35:
                    ...
                    daily_vol = (
                        returns.rolling(window=35, min_periods=10).std().iloc[-1]
                    )
                    if daily_vol > 0:
    ...
    def get_cash_weights_over_time(self, system):
        ...
        try:
            ...
            print(
                f"📊 Processing {len(dates)} time periods for {len(instruments)} instruments"
            )
            print(
                "📈 Collecting: Cash Weights, Positions, Vol Scalars, Implied Vol, Manual Vol"
            )
            ...
            for instrument in instruments:
                ...
                try:
                    ...
                    if vol_scalar > 0:
                        ...
                        vol_scalars_cache[instrument] = pd.Series(
                            [vol_scalar], index=[dates[-1]]
                        )
            ...
            for i, date in enumerate(dates):
                if i % 20 == 0:  # Progress indicator
                    print(
                        f"⏳ Processing date {i + 1}/{len(dates)}: {date.strftime('%Y-%m-%d')}"
                    )
                ...
                for instrument in instruments:
                    ...
                    try:
                        ...
                        if vol_scalars_cache.get(instrument) is not None:
                            vol_scalar = self._get_volatility_scalar_safe(
                                system, instrument
                            )
                        ...
                        if manual_vol_cache.get(instrument) is not None:
                            ...
                            try:
                                if date in manual_vol_series.index:
                                    ...
                                    # Find closest date
                                    available_dates = manual_vol_series.index[
                                        manual_vol_series.index <= date
                                    ]
                                ...
                                if pd.notna(manual_vol):
                                    manual_volatility_data[instrument].append(
                                        manual_vol
                                    )
                                else:
            ...
            volatility_scalar_df = pd.DataFrame(
                volatility_scalar_data, index=valid_dates
            )
            implied_volatility_df = pd.DataFrame(
                implied_volatility_data, index=valid_dates
            )
            manual_volatility_df = pd.DataFrame(
                manual_volatility_data, index=valid_dates
            )
            portfolio_values_series = pd.Series(portfolio_values, index=valid_dates)

            print(f"✅ Successfully calculated time series data")
            print(
                f"📊 Shape: {cash_weights_df.shape[0]} dates x {cash_weights_df.shape[1]} instruments"
            )
            print(f"📈 Volatility data coverage:")
            print(
                f"  Vol scalars: {volatility_scalar_df.notna().sum().sum()} data points"
            )
            print(
                f"  Implied vol: {implied_volatility_df.notna().sum().sum()} data points"
            )
            print(
                f"  Manual vol: {manual_volatility_df.notna().sum().sum()} data points"
            )

            # Debug: Check if we have any non-zero values
            total_nonzero_weights = (cash_weights_df != 0).sum().sum()
            total_nonzero_scalars = (
                (volatility_scalar_df.notna() & (volatility_scalar_df != 0)).sum().sum()
            )
            print(
                f"🔍 DEBUG: Non-zero weights: {total_nonzero_weights}, Non-zero scalars: {total_nonzero_scalars}"
            )
    ...
    def verify_risk_to_cash_conversion(self, system):
        ...
        if comparison_data:
            ...
            print(
                "💡 Key Insight: Cash weights ≠ Risk weights due to volatility scaling"
            )
    ...
    def export_instrument_performance_excel(
        self, filename="instrument_performance_analysis.xlsx"
    ):
        """
        ...
        with pd.ExcelWriter(filename, engine="xlsxwriter") as writer:
            ...
            summary_df["AnnualReturn"] = summary_df["AnnualReturn"].apply(
                lambda x: f"{x:.2%}"
            )
            summary_df["AnnualVolatility"] = summary_df["AnnualVolatility"].apply(
                lambda x: f"{x:.2%}"
            )
            summary_df["MaxDrawdown"] = summary_df["MaxDrawdown"].apply(
                lambda x: f"{x:.2%}"
            )
            summary_df["WinRate"] = summary_df["WinRate"].apply(lambda x: f"{x:.2%}")
            summary_df["TotalPnL"] = summary_df["TotalPnL"].apply(
                lambda x: f"${x:,.2f}"
            )
            summary_df["FinalValue"] = summary_df["FinalValue"].apply(
                lambda x: f"${x:,.2f}"
            )
            ...
            if daily_returns_data:
                returns_df = pd.DataFrame.from_dict(
                    daily_returns_data, orient="columns"
                )
    ...
    def _format_instrument_performance_sheets(self, writer, workbook):
        ...
        if "Performance_Summary" in writer.sheets:
            ...
            for col_num in range(16):  # Adjust based on number of columns
                worksheet.write(
                    0, col_num, worksheet.cell(0, col_num).value, header_format
                )
        ...
        for sheet_name in [
            ...
            if sheet_name in writer.sheets:
                ...
                worksheet.set_column(
                    ...
                    12,
                    date_format
                    if sheet_name in ["Daily_Returns", "PnL_Curves"]
                    else None,
                )
    ...
    def verify_position_sizing_formula(self, system):
        ...
        try:
            ...
            vol_scalar = system.positionSize.get_volatility_scalar(
                sample_instrument
            ).iloc[-1]
            ...
            if cash_data and sample_instrument in cash_data["cash_weights"]:
                ...
                cash_weight = system.portfolio.get_instrument_weights()[
                    sample_instrument
                ].iloc[-1]

            idm = system.portfolio.get_instrument_diversification_multiplier().iloc[-1]
            final_position = system.portfolio.get_notional_position(
                sample_instrument
            ).iloc[-1]
            ...
            print(f"\nFormula Testing:")
            print(
                f"Formula 1 (CF * VS * CW * IDM): {formula1:.4f} | Match: {abs(formula1 - final_position) < 1.0}"
            )
            print(
                f"Formula 2 (CF * VS * CW): {formula2:.4f} | Match: {abs(formula2 - final_position) < 1.0}"
            )
            print(
                f"Formula 3 ((CF * VS) * CW * IDM): {formula3:.4f} | Match: {abs(formula3 - final_position) < 1.0}"
            )

            if abs(formula1 - final_position) < 1.0:
                print(
                    "✅ Standard formula working: Position = Combined_Forecast × Vol_Scalar × Cash_Weight × IDM"
                )
                return "standard"
        ...
        try:
            ...
            for instrument in instruments:
                try:
                    ...
                    vol_scalar_series = system.positionSize.get_volatility_scalar(
                        instrument
                    )
                    if vol_scalar_series is not None and len(vol_scalar_series) > 0:
                        ...
                        if date in vol_scalar_series.index:
                            ...
                            available_dates = vol_scalar_series.index[
                                vol_scalar_series.index <= date
                            ]
                            if len(available_dates) > 0:
                                closest_date = available_dates[-1]
                        ...
                        if vol_scalar is not None and vol_scalar > 0:
                            ...
                            notional_position = (
                                fundamental_weight * system_portfolio_value
                            )
                            actual_cash_positions[instrument] = notional_position
                        else:
                            fundamental_cash_weights[instrument] = 0
            ...
            for instrument, weight in normalized_cash_weights.items():
                if weight > 0.01:  # Only print significant positions
                    cash_pos = actual_cash_positions.get(instrument, 0)
                    vol_scalar = (
                        fundamental_cash_weights[instrument]
                        / risk_weights.get(instrument, 1)
                        if risk_weights.get(instrument, 0) > 0
                        else 0
                    )
                    print(
                        f"  {instrument}: {weight:.2%} (${cash_pos:,.0f}) [vol_scalar: {vol_scalar:.2f}]"
                    )
    ...
    def export_cash_weights_time_series_excel(
        self, filename="cash_weights_time_series.xlsx"
    ):
        """
            ...
            with pd.ExcelWriter(filename, engine="xlsxwriter") as writer:
                ...
                if current_comparison:
                    current_df = pd.DataFrame.from_dict(
                        current_comparison, orient="index"
                    )
                    current_df["weight_difference"] = (
                        current_df["cash_weight"] - current_df["risk_weight"]
                    )
                ...
                vol_scalar_df.to_excel(
                    writer, sheet_name="Volatility_Scalars_TimeSeries"
                )

                # NEW Sheet 5: Implied Volatility Time Series
                implied_vol_df = time_series_data["implied_volatility_df"]
                implied_vol_df.to_excel(
                    writer, sheet_name="Implied_Volatility_TimeSeries"
                )

                # NEW Sheet 6: Manual Volatility Time Series
                manual_vol_df = time_series_data["manual_volatility_df"]
                manual_vol_df.to_excel(
                    writer, sheet_name="Manual_Volatility_TimeSeries"
                )
                ...
                portfolio_df = pd.DataFrame(
                    {
                        ...
                        "Total_Portfolio_Value": time_series_data[
                            "portfolio_values_series"
                        ].values,
                    }
                )
                portfolio_df.to_excel(
                    writer, sheet_name="Portfolio_Value_TimeSeries", index=False
                )
                ...
                for instrument in weights_df.columns:
                    ...
                    if len(weights_series) > 0:
                        summary_stats[instrument] = {
                            ...
                            "Current_Weight": weights_series.iloc[-1]
                            if len(weights_series) > 0
                            else 0,
                            "Days_Active": (weights_series > 0.001).sum(),
                            "Weight_Volatility": weights_series.std()
                            / weights_series.mean()
                            if weights_series.mean() > 0
                            else 0,
                            # NEW: Volatility statistics
                            "Mean_Vol_Scalar": vol_scalar_series.mean()
                            if len(vol_scalar_series) > 0
                            else None,
                            "Std_Vol_Scalar": vol_scalar_series.std()
                            if len(vol_scalar_series) > 0
                            else None,
                            "Current_Vol_Scalar": vol_scalar_series.iloc[-1]
                            if len(vol_scalar_series) > 0
                            else None,
                            "Mean_Implied_Vol": implied_vol_series.mean()
                            if len(implied_vol_series) > 0
                            else None,
                            "Current_Implied_Vol": implied_vol_series.iloc[-1]
                            if len(implied_vol_series) > 0
                            else None,
                            "Mean_Manual_Vol": manual_vol_series.mean()
                            if len(manual_vol_series) > 0
                            else None,
                            "Current_Manual_Vol": manual_vol_series.iloc[-1]
                            if len(manual_vol_series) > 0
                            else None,
                            "Vol_Data_Coverage": len(vol_scalar_series)
                            / len(weights_df)
                            if len(weights_df) > 0
                            else 0,
                        }
            ...
            for instrument in instruments:
                ...
                try:
                    ...
                    if vol_scalar_series is not None and len(vol_scalar_series) > 0:
                        ...
                        print(
                            f"    ✅ Vol scalars: {len(vol_scalar_series)} data points"
                        )
                    else:
                        ...
                        if vol_scalars_cache.get(instrument) is not None:
                            ...
                            try:
                                ...
                                if pd.notna(vol_scalar) and vol_scalar > 0:
                                    volatility_scalar_data[instrument].append(
                                        vol_scalar
                                    )
                                    # Calculate implied volatility
                                    implied_vol = vol_target / vol_scalar
                                    implied_volatility_data[instrument].append(
                                        implied_vol
                                    )
                                else:
                                    volatility_scalar_data[instrument].append(None)
        ...
        for instrument in instruments:
            try:
                ...
                if vol_scalar is not None:
                    ...
                    implied_annual_vol = (
                        0.12 / latest_scalar if latest_scalar > 0 else None
                    )
                else:
                    latest_scalar = None
                ...
                try:
                    ...
                    if vol_scalar_series is not None and len(vol_scalar_series) > 0:
                        if date in vol_scalar_series.index:
                            ...
                            vol_scalar = (
                                vol_scalar_series.loc[available_dates[-1]]
                                if len(available_dates) > 0
                                else None
                            )
    ...
    def _get_detailed_volatility_info(self, system, instrument, vol_target):
        ...
        try:
            ...
            try:
                ...
                daily_vol_series = system.rawdata.get_daily_percentage_volatility(
                    instrument
                )
                if daily_vol_series is not None and len(daily_vol_series) > 0:
                    ...
                    if len(latest_daily_vol) > 0:
                        ...
                        vol_info["annual_volatility"] = (
                            annual_vol_pct / 100.0
                        )  # CONVERT TO DECIMAL

                        # Calculate scalar if we don't have it from Method 1
                        if (
                            vol_info["volatility_scalar"] is None
                            and vol_info["annual_volatility"] > 0
                        ):
                            vol_info["volatility_scalar"] = (
                                vol_target / vol_info["annual_volatility"]
                            )
            except:
                pass
                ...
                try:
                    ...
                    if prices is not None and len(prices) > 35:
                        ...
                        if len(returns) > 35:
                            ...
                            daily_vol = (
                                returns.ewm(span=35, min_periods=10).std().iloc[-1]
                            )
                            ...
                            vol_info["annual_volatility"] = daily_vol * (
                                252**0.5
                            )  # Already in decimal since returns are decimal
    ...
    def export_final_day_backtest_report(
        self,
        filename="final_day_backtest_report.xlsx",
        timeseries_instruments=None,
        timeseries_days=None,
    ):
        """
        ...
        try:
            ...
            if timeseries_instruments is None:
                ...
                print(
                    f"📈 Auto-selected {timeseries_instruments[0]} for timeseries analysis"
                )
            ...
            print(
                f"📈 Will create timeseries for: {', '.join(valid_timeseries_instruments)}"
            )
            ...
            # Get target volatility from config
            target_vol = (
                getattr(self.system.config, "percentage_vol_target", 12.0) / 100
            )
            ...
            for instrument in instruments:
                try:
                    row_data = self._process_instrument_final_day(
                        instrument,
                        final_date,
                        portfolio_value,
                        target_vol,
                    ...
                    final_day_data.append(
                        {
                            ...
                            **{
                                ...
                                for k in [
                                    ...
                                    "RiskWeightConfig",
                                    "DailyVolatilityPct",
                                    "DailyVolatilityDecimal",
                                    "AnnualVolatilityPct",
                                    "AnnualVolatilityDecimal",
                                    "TargetVolatility",
                                    "CombinedForecast",
                                    "IDM",
                                    "InstrumentValueVolatility",
                                    "SubsystemPosition",
                                    "NotionalPosition",
                                    "PositionValue",
                                    "LeverageContribution",
                                    "RiskExposure",
                                    "NativeVolatilityScalarPySystemTrade",
                                    "NativeVolatilityScalarSource",
            ...
            for timeseries_instrument in valid_timeseries_instruments:
                ...
                print(
                    f"📊 Generating {len(timeseries_dates)} days of timeseries data for {timeseries_instrument}..."
                )
                ...
                for date in timeseries_dates:
                    try:
                        ...
                        date_daily_cash_vol_target = (
                            date_portfolio_value * target_vol / 16
                        )
                        ...
                        print(
                            f"⚠️ Error processing {timeseries_instrument} on {date}: {e}"
                        )
                        continue

                # Store the timeseries data for this instrument
                if timeseries_data:
                    timeseries_dataframes[timeseries_instrument] = pd.DataFrame(
                        timeseries_data
                    )
                    print(
                        f"✅ Generated {len(timeseries_data)} rows for {timeseries_instrument}"
                    )
                else:
                    print(
                        f"⚠️ No timeseries data generated for {timeseries_instrument}"
                    )
            ...
            for instrument, df_ts in timeseries_dataframes.items():
                timeseries_dataframes_excel[instrument] = df_ts.rename(
            ...
            if leverage_analysis:
                ...
                leverage_summary = [
                    ...
                    {
                        "Metric": "Total Position Value",
                        "Value": f"${leverage_metrics['total_position_value']:,.2f}",
                    },
                    {
                        "Metric": "Actual Leverage Ratio",
                        "Value": f"{leverage_metrics['actual_leverage_ratio']:.2f}x",
                    },
                    {
                        "Metric": "Capital Multiplier",
                        "Value": f"{leverage_metrics['capital_multiplier']:.2f}x",
                    },
                    {
                        "Metric": "Risk-Adjusted Leverage",
                        "Value": f"{leverage_metrics['risk_leverage_ratio']:.3f}x",
                    },
                    {
                        "Metric": "Leverage Status",
                        "Value": leverage_metrics["leverage_status"],
                    },
                ]
            ...
            with pd.ExcelWriter(filename, engine="xlsxwriter") as writer:
                ...
                df_final_excel.to_excel(
                    writer, sheet_name="Final_Day_Report", index=False
                )
                ...
                for instrument, df_ts_excel in timeseries_dataframes_excel.items():
                    ...
                    print(
                        f"✅ Created timeseries sheet: {sheet_name} with {len(df_ts_excel)} rows"
                    )

                # Leverage Analysis Sheets
                leverage_df.to_excel(
                    writer, sheet_name="Leverage_Analysis", index=False
                )
                controls_df.to_excel(
                    writer, sheet_name="Leverage_Controls", index=False
                )

                # Apply IDENTICAL formatting to all sheets
                self._format_excel_sheets_multiple(
                    writer, workbook, timeseries_sheet_names
                )
            ...
            print(
                f"📈 Created {len(timeseries_sheet_names)} timeseries sheets: {', '.join(timeseries_sheet_names)}"
            )
    ...
    def _format_excel_sheets_multiple(self, writer, workbook, timeseries_sheet_names):
        ...
        highlight_large_number_format = workbook.add_format(
            {"num_format": "#,##0.00", "bg_color": "#FFFF99"}
        )
        highlight_decimal_format = workbook.add_format(
            {"num_format": "0.0000", "bg_color": "#FFFF99"}
        )
        ...
        if "Final_Day_Report" in writer.sheets:
            self._apply_standard_formatting(
                writer.sheets["Final_Day_Report"],
                currency_format,
                percent_format,
                decimal_format,
                pct_number_format,
                large_number_format,
                date_format,
                highlight_large_number_format,
                highlight_decimal_format,
    ...
    def _apply_standard_formatting(
        ...
        worksheet.set_column(
            "J:J", 15, currency_format
        )  # Daily Cash Vol Target (FIXED)
        worksheet.set_column("K:K", 15, currency_format)  # Portfolio Value
        worksheet.set_column(
            "L:L", 15, highlight_large_number_format
        )  # Combined Forecast
        worksheet.set_column("M:M", 12, highlight_decimal_format)  # IDM
        worksheet.set_column(
            "N:N", 15, highlight_currency_format
        )  # Instrument Value Vol
    ...
    def _process_instrument_final_day(
        self, instrument, final_date, portfolio_value, target_vol, daily_cash_vol_target
    ):
        ...
        try:
            ...
            if forecast_series is not None and len(forecast_series) > 0:
                if final_date in forecast_series.index:
                    ...
                    available_dates = forecast_series.index[
                        forecast_series.index <= final_date
                    ]
                    if len(available_dates) > 0:
                        row_data["CombinedForecast"] = forecast_series.loc[
                            available_dates[-1]
                        ]
        except:
            pass
        ...
        try:
            ...
            if daily_pct_vol_series is not None and len(daily_pct_vol_series) > 0:
                if final_date in daily_pct_vol_series.index:
                    ...
                    available_dates = daily_pct_vol_series.index[
                        daily_pct_vol_series.index <= final_date
                    ]
                    if len(available_dates) > 0:
        ...
        try:
            pst_daily_vol_series = self.system.rawdata.daily_returns_volatility(
                instrument
            )
            if pst_daily_vol_series is not None and len(pst_daily_vol_series) > 0:
                if final_date in pst_daily_vol_series.index:
                    ...
                    available_dates = pst_daily_vol_series.index[
                        pst_daily_vol_series.index <= final_date
                    ]
                    if len(available_dates) > 0:
        ...
        try:
            idm_series = (
                self.system.portfolio.get_instrument_diversification_multiplier()
            )
        ...
        try:
            ...
            if subsystem_series is not None and len(subsystem_series) > 0:
                if final_date in subsystem_series.index:
                    ...
                    available_dates = subsystem_series.index[
                        subsystem_series.index <= final_date
                    ]
                    if len(available_dates) > 0:
                        row_data["SubsystemPosition"] = subsystem_series.loc[
                            available_dates[-1]
                        ]
        except:
            pass
        ...
        try:
            ...
            if notional_series is not None and len(notional_series) > 0:
                if final_date in notional_series.index:
                    ...
                    available_dates = notional_series.index[
                        notional_series.index <= final_date
                    ]
                    if len(available_dates) > 0:
                        row_data["NotionalPosition"] = notional_series.loc[
                            available_dates[-1]
                        ]
        except:
            pass
        ...
        try:
            if (
                pd.notna(row_data["ClosePrice"])
                and pd.notna(row_data["DailyVolatilityDecimal"])
                and row_data["ClosePrice"] > 0
                and row_data["DailyVolatilityDecimal"] > 0
            ):
                daily_price_vol = (
                    row_data["ClosePrice"] * row_data["DailyVolatilityDecimal"]
                )
        ...
        try:
            if (
                pd.notna(row_data["NotionalPosition"])
                and pd.notna(row_data["ClosePrice"])
                and row_data["ClosePrice"] > 0
            ):
                position_value = (
                    abs(row_data["NotionalPosition"]) * row_data["ClosePrice"]
                )
        ...
        try:
            if pd.notna(row_data["PositionValue"]) and portfolio_value > 0:
                row_data["LeverageContribution"] = (
                    row_data["PositionValue"] / portfolio_value
                )
        except:
            pass
        ...
        try:
            if (
                pd.notna(row_data["PositionValue"])
                and pd.notna(row_data["DailyVolatilityDecimal"])
                and row_data["DailyVolatilityDecimal"] > 0
            ):
                risk_exposure = (
                    row_data["PositionValue"] * row_data["DailyVolatilityDecimal"]
                )
        ...
        try:
            (
                native_vol_scalar,
                native_vol_scalar_source,
            ) = self.get_volatility_scalar_safe(self.system, instrument)
    ...
    def _process_instrument_timeseries_day(
        self, instrument, date, portfolio_value, target_vol, daily_cash_vol_target
    ):
        ...
        try:
            ...
            if forecast_series is not None and len(forecast_series) > 0:
                if date in forecast_series.index:
                    ...
                    available_dates_forecast = forecast_series.index[
                        forecast_series.index <= date
                    ]
                    if len(available_dates_forecast) > 0:
                        row_data["CombinedForecast"] = forecast_series.loc[
                            available_dates_forecast[-1]
                        ]
        except:
            pass
        ...
        try:
            ...
            if daily_pct_vol_series is not None and len(daily_pct_vol_series) > 0:
                if date in daily_pct_vol_series.index:
                    ...
                    available_dates_vol = daily_pct_vol_series.index[
                        daily_pct_vol_series.index <= date
                    ]
                    if len(available_dates_vol) > 0:
                        daily_pct_vol = daily_pct_vol_series.loc[
                            available_dates_vol[-1]
                        ]
                    else:
                        daily_pct_vol = None
                    ...
                    if len(prices_up_to_date) > 35:
                        ...
                        if len(returns) > 35:
                            ...
                            if pd.notna(daily_vol_decimal) and daily_vol_decimal > 0:
                                ...
                                annual_vol_decimal = daily_vol_decimal * (252**0.5)
                                row_data["AnnualVolatilityDecimal"] = annual_vol_decimal
                                row_data["AnnualVolatilityPct"] = (
                                    annual_vol_decimal * 100
                                )
            except:
                pass
        ...
        try:
            ...
            if pst_daily_vol_series is not None and len(pst_daily_vol_series) > 0:
                if date in pst_daily_vol_series.index:
                    ...
                    available_dates_pst = pst_daily_vol_series.index[
                        pst_daily_vol_series.index <= date
                    ]
                    if len(available_dates_pst) > 0:
                        pst_daily_vol = pst_daily_vol_series.loc[
                            available_dates_pst[-1]
                        ]
                    else:
                        pst_daily_vol = None
        ...
        try:
            ...
            if subsystem_series is not None and len(subsystem_series) > 0:
                if date in subsystem_series.index:
                    ...
                    available_dates_sub = subsystem_series.index[
                        subsystem_series.index <= date
                    ]
                    if len(available_dates_sub) > 0:
                        row_data["SubsystemPosition"] = subsystem_series.loc[
                            available_dates_sub[-1]
                        ]
        except:
            pass
        ...
        try:
            ...
            if notional_series is not None and len(notional_series) > 0:
                if date in notional_series.index:
                    ...
                    available_dates_not = notional_series.index[
                        notional_series.index <= date
                    ]
                    if len(available_dates_not) > 0:
                        row_data["NotionalPosition"] = notional_series.loc[
                            available_dates_not[-1]
                        ]
        except:
            pass
        ...
        try:
            ...
            try:
                ...
                if vol_scalar_series is not None and len(vol_scalar_series) > 0:
                    ...
                    if date in vol_scalar_series.index:
                        ...
                        available_dates_vs = vol_scalar_series.index[
                            vol_scalar_series.index <= date
                        ]
                        if len(available_dates_vs) > 0:
                            vol_scalar_extracted = vol_scalar_series.loc[
                                available_dates_vs[-1]
                            ]
            ...
            if vol_scalar_extracted is None or vol_scalar_extracted <= 0:
                try:
                    ...
                    if (
                        subsystem_pos is not None
                        and forecast_val is not None
                        and not pd.isna(subsystem_pos)
                        and not pd.isna(forecast_val)
                        and abs(forecast_val) > 0.01
                    ):
            ...
            print(
                f"⚠️ Native volatility scalar extraction failed for {instrument} on {date}: {e}"
            )
            row_data["NativeVolatilityScalarPySystemTrade"] = np.nan
            row_data["NativeVolatilityScalarSource"] = "ERROR"
    ...
    def _extract_position_fields(self, row_data, instrument, date):
        ...
        try:
            if not pd.isna(row_data["NotionalPosition"]) and not pd.isna(
                row_data["ClosePrice"]
            ):
                row_data["PositionValue"] = (
                    abs(row_data["NotionalPosition"]) * row_data["ClosePrice"]
                )

                # Calculate leverage contribution
                if (
                    not pd.isna(row_data["PortfolioValue"])
                    and row_data["PortfolioValue"] > 0
                ):
                    row_data["LeverageContribution"] = (
                        row_data["PositionValue"] / row_data["PortfolioValue"]
                    )

                # Calculate risk exposure
                if not pd.isna(row_data["DailyVolatilityDecimal"]):
                    row_data["RiskExposure"] = (
                        row_data["PositionValue"] * row_data["DailyVolatilityDecimal"]
                    )
        except:
            pass
    ...
    def calculate_actual_leverage_metrics(self, system, date=None):
        ...
        try:
                ...
                try:
                    ...
                    if portfolio_pos is not None and len(portfolio_pos) > 0:
                        if date in portfolio_pos.index:
                            ...
                            available_dates = portfolio_pos.index[
                                portfolio_pos.index <= date
                            ]
                            position_size = (
                                portfolio_pos.loc[available_dates[-1]]
                                if len(available_dates) > 0
                                else 0
                            )
                    else:
                    ...
                    position_value = (
                        abs(position_size * current_price) if current_price > 0 else 0
                    )
                    ...
                    leverage_data[instrument] = {
                        ...
                        "leverage_contribution": position_value / actual_trading_capital
                        if actual_trading_capital > 0
                        else 0,
                    }
            ...
            actual_leverage_ratio = (
                total_position_value / actual_trading_capital
                if actual_trading_capital > 0
                else 0
            )
            capital_multiplier = (
                actual_leverage_ratio  # Carver's capital multiplier concept
            )
            risk_leverage_ratio = (
                total_risk_exposure / actual_trading_capital
                if actual_trading_capital > 0
                else 0
            )
            ...
            leverage_status, status_icon = assess_leverage_status(
                actual_leverage_ratio, risk_leverage_ratio
            )
            ...
            print(
                f"{'Instrument':<15} {'Position':<12} {'Value':<12} {'Leverage':<10} {'Risk Exp':<12}"
            )
            print("-" * 70)
            ...
            for instrument, data in leverage_data.items():
                if data["position_value"] > 1000:  # Only show significant positions
                    print(
                        f"{instrument:<15} {data['position_size']:<12.1f} "
    ...
    def add_leverage_controls_check(self, system):
        ...
        if max_position_pct <= 0.3:
            ...
            ] = "⚠️ CAUTION - Some concentration"
        else:
            controls_status[
                "position_concentration"
            ] = "🚨 FAIL - High concentration risk"
    ...
    def export_diagnostic_analysis_excel(self, filename="diagnostic_analysis.xlsx"):
        ...
        try:
            ...
            focus_instruments = (
                ["BBAX", "IVV", "HYD"]
                if all(inst in instruments for inst in ["BBAX", "IVV", "HYD"])
                else instruments[:3]
            )
            ...
            for instrument in focus_instruments:
                ...
                try:
                    ...
                    if prices is not None and len(prices) > 0:
                        ...
                        diagnostic_info["DataCompleteness"] = (
                            len(prices) - missing_count
                        ) / len(prices)
                        ...
                        diagnostic_info["CorporateActions"] = len(
                            very_large_moves
                        )  # Proxy for corporate actions
                except Exception as e:
                ...
                try:
                    ...
                    system_vol_series = (
                        self.system.rawdata.get_daily_percentage_volatility(instrument)
                    )
                    if system_vol_series is not None and len(system_vol_series) > 0:
                        system_vol = (
                            system_vol_series.iloc[-1] / 100
                        )  # Convert to decimal
                    ...
                    pst_vol_series = self.system.rawdata.daily_returns_volatility(
                        instrument
                    )
                    ...
                    print(
                        f"⚠️ System volatility extraction failed for {instrument}: {e}"
                    )

                # 3. FORECAST SCALAR CHECK
                try:
                    # Check if using fixed or estimated forecast scalars
                    use_estimates = getattr(
                        self.system.config, "use_forecast_scale_estimates", False
                    )
                    print(
                        f"📊 {instrument} - Using estimated forecast scalars: {use_estimates}"
                    )

                    if use_estimates:
                        # Estimated scalars (instrument-specific)
                        scalar_series = (
                            self.system.forecastScaleCap._get_forecast_scalar_estimated(
                                instrument, "ewmac8"
                            )
                        )
                        ...
                        try:
                            scalar = self.system.config.trading_rules["ewmac8"][
                                "forecast_scalar"
                            ]
                            ...
                            try:
                                ...
                                scalar = self.system.config.get_element(
                                    "forecast_scalar"
                                )
                                diagnostic_info["ForecastScalar"] = scalar
                ...
                try:
                    ...
                    block_move = self.system.rawdata.get_value_of_block_price_move(
                        instrument
                    )
                    if hasattr(block_move, "iloc"):
                        diagnostic_info["LatestBlockValue"] = float(block_move.iloc[-1])
                    else:
                        diagnostic_info["LatestBlockValue"] = (
                            float(block_move) if block_move else 1.0
                        )

                    # FX Rate check
                    try:
                        fx_rate = self.system.rawdata.get_fx_for_instrument(
                            instrument, "USD"
                        )
                        if hasattr(fx_rate, "iloc"):
                            diagnostic_info["FXRate"] = float(fx_rate.iloc[-1])
                        else:
                            diagnostic_info["FXRate"] = (
                                float(fx_rate) if fx_rate else 1.0
                            )
                    except:
            ...
            for instrument in focus_instruments:
                try:
                    # Get system volatility scalar
                    system_scalar, scalar_source = self.get_volatility_scalar_safe(
                        self.system, instrument
                    )
                    ...
                    if pd.notna(manual_vol) and manual_vol > 0:
                        ...
                        missing_factor = (
                            system_scalar / expected_scalar
                            if expected_scalar > 0
                            else np.nan
                        )
                    else:
                    ...
                    vol_scalar_comparison[instrument] = {
                        ...
                        "ManualAnnualVol": manual_vol * (252**0.5)
                        if pd.notna(manual_vol)
                        else np.nan,
                    ...
                    print(
                        f"⚠️ Volatility scalar comparison failed for {instrument}: {e}"
                    )
            ...
            with pd.ExcelWriter(filename, engine="xlsxwriter") as writer:
                ...
                data_quality_df = pd.DataFrame.from_dict(
                    diagnostic_data, orient="index"
                )
                data_quality_df.to_excel(writer, sheet_name="Data_Quality", index=False)

                # Sheet 2: Volatility Scalar Comparison
                vol_scalar_df = pd.DataFrame.from_dict(
                    vol_scalar_comparison, orient="index"
                )
                vol_scalar_df.to_excel(
                    writer, sheet_name="Volatility_Scalar_Analysis", index=False
                )
                ...
                portfolio_curve = portfolio.curve()
                recent_dates = (
                    portfolio_curve.index[-50:]
                    if len(portfolio_curve) >= 50
                    else portfolio_curve.index
                )
                ...
                for date in recent_dates:
                    for instrument in focus_instruments:
                        try:
                            ...
                            prices = self.system.rawdata.get_daily_prices(instrument)
                            close_price = (
                                prices.loc[date] if date in prices.index else np.nan
                            )

                            # Get returns up to this date for volatility calculation
                            returns_up_to_date = prices.pct_change().dropna()
                            returns_up_to_date = returns_up_to_date[
                                returns_up_to_date.index <= date
                            ]

                            if len(returns_up_to_date) > 35:
                                rolling_vol = (
                                    returns_up_to_date.ewm(span=35).std().iloc[-1]
                                )
                            else:
                                rolling_vol = np.nan
                            ...
                            try:
                                vol_scalar_series = (
                                    self.system.positionSize.get_volatility_scalar(
                                        instrument
                                    )
                                )
                                if (
                                    vol_scalar_series is not None
                                    and date in vol_scalar_series.index
                                ):
                            ...
                            timeseries_comparison.append(
                                {
                                    ...
                                    "ExpectedVolScalar": target_vol
                                    / (rolling_vol * (252**0.5))
                                    if pd.notna(rolling_vol) and rolling_vol > 0
                                    else np.nan,
                                }
                            )
                ...
                if len(timeseries_df) > 0:
                    ...
                    for metric in [
                        ...
                        pivot_df = timeseries_df.pivot(
                            index="Date", columns="Instrument", values=metric
                        )
    ...
    def verify_forecast_scalar_consistency(self, system):
        ...
        for rule in trading_rules:
            ...
            for instrument in instruments:
                try:
                    ...
                    if rule in instrument_rules:
                        ...
                        if (
                            hasattr(system.config, "use_forecast_scale_estimates")
                            and system.config.use_forecast_scale_estimates
                        ):
                            # Using estimated scalars
                            scalar_series = (
                                system.forecastScaleCap._get_forecast_scalar_estimated(
                                    instrument, rule
                                )
                            )
                            if scalar_series is not None and len(scalar_series) > 0:
                            ...
                            # Using fixed scalars
                            scalar_value = (
                                system.forecastScaleCap._get_forecast_scalar_fixed(
                                    instrument, rule
                                )
                            )
                            scalar_type = "FIXED"
            ...
            scalar_values = [
                data["scalar"]
                for data in rule_scalars.values()
                if not pd.isna(data["scalar"])
            ]
            if len(scalar_values) > 1:
                ...
                print(
                    f"    Mean: {scalar_mean:.4f}, Std: {scalar_std:.4f}, CV: {cv:.4f}"
                )

                if cv > 0.01:  # More than 1% variation
                    print(f"    ⚠️ HIGH VARIATION DETECTED! ({cv:.2%})")
                    print(
                        f"    🔍 This could explain your Missing Factor discrepancies!"
                    )
    ...
    def createcomprehensiveequitycurveplot(self):
        ...
        try:
            ...
            daily_returns = fixed_capital_pnl.pct_change().fillna(
                0
            )  # Use return percentages
            ...
            print(
                f"Simulated Compounding Final: ${simulated_compounding_equity.iloc[-1]:,.2f}"
            )
            print(f"Difference: ${difference_precise.iloc[-1]:,.2f}")
            print(
                f"Difference Range: ${difference_precise.min():,.2f} to ${difference_precise.max():,.2f}"
            )
            ...
            print(
                f"Annualized Return: {(total_return / (len(daily_returns) / 252)):.2%}"
            )
            ...
            ax1.plot(
                ...
                "b-",
                linewidth=3,
                label=f"Fixed Capital (Actual Backtest)",
                alpha=0.8,
            )
            ax1.plot(
                simulated_compounding_equity.index,
                simulated_compounding_equity.values,
                "r--",
                linewidth=2,
                label=f"Simulated Compounding (What-If)",
                alpha=0.8,
            )
            ax1.set_title(
            ...
            ax2.plot(
                compounding_multipliers.index,
                compounding_multipliers.values,
                "g-",
                linewidth=2,
            )
            ax2.axhline(
                y=1.0, color="black", linestyle="--", alpha=0.5, label="Initial Level"
            )
            ax2.set_title(
                f"Compounding Multiplier (Final: {compounding_multipliers.iloc[-1]:.4f})",
                fontsize=14,
                fontweight="bold",
            )
            ...
            ax3.plot(
                difference_precise.index,
                difference_precise.values,
                "purple",
                linewidth=2,
            )
            ...
            max_abs_diff = max(
                abs(difference_precise.min()), abs(difference_precise.max())
            )
            if max_abs_diff < 10:
    ...
    def create_strategy_comparison_plot(self):
        ...
        try:
            ...
            try:
                ...
                # Align dates
                common_dates = ls_equity.index.intersection(
                    ivv_equity.index
                ).intersection(hyd_equity.index)
            ...
            fig.canvas.manager.set_window_title(
                "Enhanced Strategy Comparison - Keep Open!"
            )
            fig.suptitle(
                "🏆 SYSTEMATIC TRADING vs BUY & HOLD COMPARISON",
                fontsize=20,
                fontweight="bold",
                y=0.95,
            )
            ...
            ax1.plot(
                ls_equity_aligned.index,
                ls_equity_aligned.values,
                "b-",
                linewidth=4,
            ...
            ax1.plot(
                lo_equity_aligned.index,
                lo_equity_aligned.values,
                "r-",
                linewidth=4,
            ...
            ax1.plot(
                ivv_equity_aligned.index,
                ivv_equity_aligned.values,
                "g-",
                linewidth=3,
                label="🟢 Buy & Hold IVV (S&P 500)",
                alpha=0.8,
            )
            ax1.plot(
                hyd_equity_aligned.index,
                hyd_equity_aligned.values,
                "orange",
                linewidth=3,
                label="🟠 Buy & Hold HYD (Muni Bonds)",
                alpha=0.8,
            )

            ax1.set_title(
                "📊 PORTFOLIO VALUE COMPARISON", fontsize=16, fontweight="bold", pad=20
            )
            ...
            ax2.plot(
                ...
                "purple",
                linewidth=3,
                label="🟣 Long-Short vs HYD",
                alpha=0.9,
            )
            ax2.axhline(y=0, color="black", linestyle="--", alpha=0.7, linewidth=2)

            ax2.set_title(
                "📈 EXCESS RETURNS OVER BENCHMARKS", fontsize=16, fontweight="bold"
            )
            ...
            ls_vol = (
                ls_equity_aligned.pct_change().rolling(window).std()
                * np.sqrt(252)
                * 100
            )
            lo_vol = (
                lo_equity_aligned.pct_change().rolling(window).std()
                * np.sqrt(252)
                * 100
            )
            ivv_vol = (
                ivv_equity_aligned.pct_change().rolling(window).std()
                * np.sqrt(252)
                * 100
            )
            hyd_vol = (
                hyd_equity_aligned.pct_change().rolling(window).std()
                * np.sqrt(252)
                * 100
            )

            ax3.plot(
                ls_vol.index, ls_vol.values, "b-", linewidth=3, label="🔵 Long-Short"
            )
            ax3.plot(
                lo_vol.index, lo_vol.values, "r-", linewidth=3, label="🔴 Long-Only"
            )
            ax3.plot(ivv_vol.index, ivv_vol.values, "g-", linewidth=3, label="🟢 IVV")
            ax3.plot(
                hyd_vol.index, hyd_vol.values, "orange", linewidth=3, label="🟠 HYD"
            )

            ax3.set_title(
                f"📊 {window}-DAY ROLLING VOLATILITY", fontsize=16, fontweight="bold"
            )
            ...
            for name, equity in strategies.items():
                ...
                annual_return = (
                    (equity.iloc[-1] / starting_capital) ** (252 / len(equity)) - 1
                ) * 100
            ...
            for stats in stats_summary:
                summary_lines.append(
                    f"{stats['Strategy']}: Ret {stats['Total Return']}, "
            ...
            for stats in stats_summary:
                print(
                    f"{stats['Strategy']:15} | {stats['Total Return']:8} | {stats['Annual Return']:8} | "
    ...
    def create_simple_strategy_comparison(self, ls_equity, lo_equity):
        ...
        try:
            ...
            ax.set_title(
                "🏆 Strategy Comparison: Long-Short vs Long-Only",
                fontsize=16,
                fontweight="bold",
            )
    ...
    def calculate_buy_and_hold_benchmark(
        self, symbol, start_date, end_date, starting_capital
    ):
    ...
    def calculate_long_only_pnl(self):
        ...
        try:
            ...
            for instrument in instruments:
                ...
                instrument_pnl = (
                    long_only_positions.shift(1) * price_changes * prices.shift(1)
                ).fillna(0)
            ...
            if daily_pnl_series:
                ...
                print(
                    f"Long-only P&L calculation completed: {len(portfolio_cumulative_pnl)} data points"
                )
