...
class PostCompletionAnalyzer:
    ...
    def _analyze_performance(self):
        ...
        if not self.performance_metrics:
            return {'status': 'UNAVAILABLE', 'message': 'Performance calculation not completed'}
    ...
    def _display_executive_summary(self, report):
        ...
        if exec_status == 'SUCCESS':
            print(f"   • Instruments: {report['execution_summary']['instruments_processed']}")
            print(f"   • Trading Rules: {report['execution_summary']['rules_implemented']}")
            print(f"   • Total Combinations: {report['execution_summary']['total_combinations']}")

        # Compliance Status
        compliance = report['carver_compliance']
        compliance_status = "✅ COMPLIANT" if compliance.get('overall_compliance') else "❌ NON-COMPLIANT"
        print(f"🎯 Carver Compliance: {compliance_status}")

        # Performance Status
        performance = report['performance_analysis']
        if performance.get('sharpe_ratio'):
            print(f"📊 Performance: Sharpe {performance['sharpe_ratio']:.3f}, "
                  f"Return {performance['annual_return']:.1%}, "
                  f"Drawdown {performance['max_drawdown']:.1%}")

        # Production Readiness
        readiness = report['production_readiness']
        print(f"🚀 Production Readiness: {readiness['status']} ({readiness['percentage']:.0f}%)")
