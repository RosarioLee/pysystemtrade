...
class EnhancedSystemRunner:
    ...
    def run_comprehensive_analysis(
        ...
        try:
            ...
            monitoring_results = self._run_monitoring_analysis(
                system, etf_system, performance_results, create_dashboards, run_stress_tests
            )
    ...
    def _initialize_system(self, test_mode: bool, max_instruments: int) -> Optional[Dict[str, Any]]:
    ...
    def _run_performance_analysis(self, system) -> Optional[Dict[str, Any]]:
        ...
        try:
            if AdvancedPerformanceCalculator is None:
                print("⚠️ AdvancedPerformanceCalculator not available, using basic analysis")
    ...
    def _run_monitoring_analysis(
        ...
        try:
            if AdvancedProductionMonitor is None:
                print("⚠️ AdvancedProductionMonitor not available, using basic monitoring")
                return self._run_basic_monitoring(system, performance_results)

            monitor = AdvancedProductionMonitor(
                system,
                etf_system.monitoring_config if hasattr(etf_system, 'monitoring_config') else {}
            )
            ...
            if create_dashboards and performance_results:
                try:
                    ...
                    dashboard_results = monitor.create_advanced_dashboards(performance_results)
                except Exception as e:
    ...
    def _generate_deployment_assessment(
        ...
        if monitoring_results and monitoring_results.get('health_check'):
            ...
            passed_checks = sum(1 for check in detailed_checks.values()
    ...
    def _save_results(self, results: Dict[str, Any]):
        ...
        try:
            ...
            summary_results = {
                ...
                'timestamp': results.get('timestamp').isoformat() if results.get('timestamp') else None,
                'configuration': results.get('configuration'),
                'monitoring_summary': {
                    'health_status': results.get('monitoring_results', {}).get('health_check', {}).get(
                        'overall_status'),
                    'stress_test_score': results.get('monitoring_results', {}).get('stress_testing', {}).get(
                        'overall_assessment', {}).get('overall_stress_score')
                }
            }
    ...
    def _display_comprehensive_summary(self, results: Dict[str, Any]):
        ...
        if system:
            ...
            print(f" • Trading Rules: {len(system.rules.trading_rules()) if hasattr(system, 'rules') else 0}")
            print(f" • Analysis Duration: {results.get('execution_time', 0):.1f} seconds")
        ...
        if deployment:
            ...
            print(f" • Score: {deployment.get('score', 0)}/{deployment.get('max_score', 100)} "
