...
class ForecastScalarMonitor:
    ...
    def __init__(self, system, config=None):
        ...
        self.carver_reference = {
            'ewmac_4_16': 8.54, 'ewmac_8_32': 5.95,
            'ewmac_16_64': 4.10, 'ewmac_32_128': 2.79,
            'breakout_10': 0.71, 'breakout_20': 0.79,
            'breakout_40': 0.82, 'breakout_80': 0.84,
            'breakout_160': 0.84, 'breakout_320': 0.83
        }
            ...
            for rule_name in self.rules.keys():
                try:
                    ...
                    if len(scalar_series) > 50:  # Minimum data for analysis
                        ...
                        scalar_analysis['current_scalars'][instrument][rule_name] = current_scalar
                        ...
                        if len(scalar_series) > 120:
                            ...
                            scalar_analysis['scalar_trends'][instrument][rule_name] = trend_change
                        ...
                        if len(scalar_series) > 252:
                            cv = scalar_series.tail(252).std() / scalar_series.tail(252).mean()
                            if instrument not in scalar_analysis['scalar_stability']:
                                scalar_analysis['scalar_stability'][instrument] = {}
                            scalar_analysis['scalar_stability'][instrument][rule_name] = cv
                        ...
                        if rule_name in self.carver_reference:
                            ...
                            deviation = abs(current_scalar - reference) / reference * 100
                            if instrument not in scalar_analysis['carver_comparison']:
                                scalar_analysis['carver_comparison'][instrument] = {}
                            scalar_analysis['carver_comparison'][instrument][rule_name] = deviation
                    ...
                    print(f"⚠️ Scalar analysis error {instrument}-{rule_name}: {str(e)[:50]}...")
        ...
        if scalar_analysis['alerts']:
            ...
            for alert in scalar_analysis['alerts'][:5]:  # Show first 5 alerts
                if alert['type'] == 'large_deviation':
                    ...
                    print(f"   • {alert['instrument']}-{alert['rule']}: High scalar volatility (CV={alert['cv']:.2f})")
    ...
    def create_scalar_monitoring_dashboard(self, scalar_analysis):
        ...
        try:
            if scalar_analysis['current_scalars']:
                ...
                rules_sample = ['ewmac_8_32', 'ewmac_16_64', 'breakout_20', 'breakout_80']
                ...
                if instruments_sample:
                    current_vals = [scalar_analysis['current_scalars'][instruments_sample[0]].get(rule, 0)
                                    for rule in rules_sample]
                    carver_vals = [self.carver_reference.get(rule, 0) for rule in rules_sample]

                    ax1.bar(x_pos - width / 2, current_vals, width, label='Current System', alpha=0.8)
                    ax1.bar(x_pos + width / 2, carver_vals, width, label='Carver Reference', alpha=0.8)
                    ax1.set_xlabel('Trading Rules')
                    ax1.set_ylabel('Forecast Scalar')
                    ax1.set_title(f'Forecast Scalars: Current vs Reference\n({instruments_sample[0]})')
                    ax1.set_xticks(x_pos)
                    ax1.set_xticklabels(rules_sample, rotation=45)
                    ax1.legend()
                    ax1.grid(True, alpha=0.3)
        except Exception as e:
            ax1.text(0.5, 0.5, f'Scalar comparison\nerror: {str(e)[:50]}...',
        ...
        try:
            if scalar_analysis['scalar_trends']:
                ...
                if trends_data:
                    colors = ['red' if t < -10 else 'green' if t > 10 else 'blue' for t in trends_data]
                    ax2.barh(range(len(trends_data)), trends_data, color=colors, alpha=0.7)
            ...
            ax2.text(0.5, 0.5, f'Trend analysis\nerror: {str(e)[:50]}...',
        ...
        try:
            if scalar_analysis['scalar_stability']:
                ...
                if stability_data:
                    colors = ['red' if cv > 0.3 else 'orange' if cv > 0.2 else 'green' for cv in stability_data]
                    ax3.bar(range(len(stability_data)), stability_data, color=colors, alpha=0.7)
                    ...
                    ax3.axhline(y=0.3, color='red', linestyle='--', alpha=0.7, label='High volatility threshold')
                    ax3.legend()
                    ax3.grid(True, alpha=0.3)
        except Exception as e:
            ax3.text(0.5, 0.5, f'Stability analysis\nerror: {str(e)[:50]}...',
...
"""
        ...
        ax4.text(0.05, 0.95, alert_summary, transform=ax4.transAxes,
                 fontsize=9, verticalalignment='top', fontfamily='monospace',
                 bbox=dict(boxstyle='round', facecolor='lightblue', alpha=0.8))

        plt.suptitle('Forecast Scalar Monitoring Dashboard', fontsize=14, fontweight='bold')
        plt.tight_layout()
        plt.show()
...
class RuleWeightMonitor:
        ...
        for instrument in sample_instruments:
            try:
                ...
                if isinstance(weights, pd.DataFrame) and len(weights) > 0:
                    ...
                    effective_rules = sum(1 for w in current_weights.values() if w > 0.05)
                ...
                for rule_name in self.rules.keys():
                    try:
                        forecast = self.system.forecastScaleCap.get_scaled_forecast(instrument, rule_name)
                        if len(forecast) > 252:
                ...
                if len(forecast_data) > 1:
                    ...
                    if len(forecast_df) > 100:
                        ...
                        weight_analysis['rule_correlations'][instrument] = correlation_matrix
                    ...
                    try:
                        ...
                        if hasattr(rule_pandl, 'sharpe'):
                            attribution[rule_name] = {
                                ...
                                'annual_return': rule_pandl.gross.resample('A').last().pct_change().mean() * 100,
                                'volatility': rule_pandl.percentage.std() * np.sqrt(252)
                            }
                    except:
                        continue
        ...
        if weight_analysis['alerts']:
            ...
            for alert in weight_analysis['alerts'][:5]:  # Show first 5 alerts
                if alert['type'] == 'high_concentration':
                    print(f"   • {alert['instrument']}: High concentration ({alert['concentration']:.1%})")
    ...
    def create_weight_tracking_dashboard(self, weight_analysis):
        ...
        try:
            ...
            ax1.text(0.5, 0.5, f'Weight allocation\nerror: {str(e)[:50]}...',
                     ha='center', va='center', transform=ax1.transAxes)
        ...
        try:
            if weight_analysis['weight_evolution']:
                ...
                if isinstance(weight_evolution, pd.DataFrame) and len(weight_evolution) > 50:
                    ...
                    for column in weight_evolution.columns[:6]:  # Show top 6 rules
                        if column in weight_evolution.columns:
                            weight_evolution[column].plot(ax=ax2, label=column, alpha=0.8)
            ...
            ax2.text(0.5, 0.5, f'Weight evolution\nerror: {str(e)[:50]}...',
                     ha='center', va='center', transform=ax2.transAxes)
        ...
        try:
            ...
            ax3.text(0.5, 0.5, f'Correlation analysis\nerror: {str(e)[:50]}...',
                     ha='center', va='center', transform=ax3.transAxes)
        ...
        try:
            if weight_analysis['performance_attribution']:
                first_instrument = list(weight_analysis['performance_attribution'].keys())[0]
                attribution = weight_analysis['performance_attribution'][first_instrument]

                if attribution:
                    rules = list(attribution.keys())
                    sharpes = [attribution[rule]['sharpe'] for rule in rules if 'sharpe' in attribution[rule]]

                    if sharpes and len(sharpes) == len(rules):
                        colors = ['green' if s > 0.5 else 'orange' if s > 0 else 'red' for s in sharpes]
                        bars = ax4.bar(range(len(rules)), sharpes, color=colors, alpha=0.7)
                        ax4.set_xticks(range(len(rules)))
                        ax4.set_xticklabels(rules, rotation=45)
                        ax4.set_ylabel('Sharpe Ratio')
                        ax4.set_title(f'Rule Performance Attribution\n({first_instrument})')
            ...
            ax4.text(0.5, 0.5, f'Performance attribution\nerror: {str(e)[:50]}...',
        ...
        try:
            if weight_analysis['diversification_metrics']:
                ...
                effective_rules = [weight_analysis['diversification_metrics'][inst]['effective_rules']
                                   for inst in instruments]
                concentrations = [weight_analysis['diversification_metrics'][inst]['concentration']
                                  for inst in instruments]

                if effective_rules and concentrations:
                    # Scatter plot of diversification
                    scatter = ax5.scatter(effective_rules, concentrations, alpha=0.7, s=100)
                    ...
                    for i, instrument in enumerate(instruments):
                        ax5.annotate(instrument[:3], (effective_rules[i], concentrations[i]),
                                     xytext=(5, 5), textcoords='offset points', fontsize=8)
        except Exception as e:
            ax5.text(0.5, 0.5, f'Diversification metrics\nerror: {str(e)[:50]}...',
        ...
        try:
            ...
            avg_effective_rules = np.mean([metrics['effective_rules']
                                           for metrics in weight_analysis['diversification_metrics'].values()])
            avg_concentration = np.mean([metrics['concentration']
                                         for metrics in weight_analysis['diversification_metrics'].values()])
...
"""
        ...
        plt.suptitle('Rule Weight Allocation Tracking Dashboard', fontsize=14, fontweight='bold')
        plt.tight_layout()
        plt.show()
...
class SystemMonitor:
    ...
    def run_full_monitoring(self):
        ...
        scalar_dashboard = self.scalar_monitor.create_scalar_monitoring_dashboard(scalar_analysis)
        ...
        weight_dashboard = self.weight_monitor.create_weight_tracking_dashboard(weight_analysis)
        ...
        monitoring_report = {
            ...
            'total_alerts': len(scalar_analysis['alerts']) + len(weight_analysis['alerts']),
            'monitoring_timestamp': datetime.now()
        }

        print(f"\n📊 MONITORING SUMMARY:")
        print(f"✅ Scalar analysis: {len(scalar_analysis['current_scalars'])} instruments")
        print(f"✅ Weight analysis: {len(weight_analysis['current_weights'])} instruments")
