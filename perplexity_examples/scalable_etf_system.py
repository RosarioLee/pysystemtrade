...
class ScalableETFSystem:
        ...
        try:
            ...
            for instrument in top_instruments:
                if instrument in system.get_instrument_list():
                    inst_idm = system.portfolio.get_instrument_diversification_multiplier(instrument)
            ...
            print(f"Max Theoretical IDM ({n_instruments} instruments): {max_theoretical_idm:.3f}")
            print(f"Current vs Max: {(idm_series.iloc[-1] / max_theoretical_idm * 100):.1f}%")
    ...
    def extract_actual_idm_from_system(self):
        ...
        try:
            ...
            actual_idm_series = pst_system.portfolio.get_instrument_diversification_multiplier()

            print(f"✅ Successfully extracted actual IDM time series")
            print(f"IDM Series Length: {len(actual_idm_series)} data points")
            print(f"Date Range: {actual_idm_series.index[0]} to {actual_idm_series.index[-1]}")
    ...
    def analyze_actual_idm_performance(self):
        ...
        try:
            ...
            for instrument in self.config.instruments:
                try:
                    pos_before = pst_system.portfolio.get_notional_position_without_idm(instrument)
                    pos_after = pst_system.portfolio.get_notional_position_before_risk_scaling(instrument)
    ...
    def extract_current_idm_simple(self):
        ...
        try:
            ...
            try:
                ...
                current_idm = pst_system.portfolio.get_instrument_diversification_multiplier().iloc[-1]
                ...
                try:
                    ...
                    try:
                        n_instruments = len(self.config.instruments)
                        default_idm = pst_system.config.get_element_or_default("instrument_div_mult",
                                                                               np.sqrt(n_instruments))
    ...
    def analyze_idm_vs_performance(self, performance):
        ...
        if idm_metrics and performance:
            ...
            print(f"   Risk-Adjusted Performance per IDM Unit: {avg_sharpe / current_idm:.3f}")
            ...
            print(f"   Potential Sharpe with Perfect Diversification: {potential_sharpe:.3f}")
    ...
    def analyze_idm_vs_performance_corrected(self, performance):
        ...
        if idm_metrics and performance:
            ...
            if current_idm > 2.5:
                ...
                print(f"   🎯 Recommendation: Implement 2.5 IDM cap for better risk control")
            else:
            ...
            return {
                ...
                'risk_adjusted_performance': avg_sharpe * (min(2.5, current_idm) / current_idm)
            }

        return None


def create_idm_analysis_plots(self, idm_series, instrument_weights,
                              positions_before_idm, positions_after_idm, pst_system):
    ...
    ax1.axhline(y=mean_idm, color='red', linestyle='--', alpha=0.7,
                label=f'Mean: {mean_idm:.3f}')
    ax1.axhline(y=idm_series.quantile(0.25), color='orange', linestyle=':', alpha=0.5,
                label=f'25th %ile: {idm_series.quantile(0.25):.3f}')
    ax1.axhline(y=idm_series.quantile(0.75), color='orange', linestyle=':', alpha=0.5,
    ...
    ax2.axhline(y=1.0, color='gray', linestyle=':', alpha=0.5, label='No Diversification (1.0)')
    ...
    ax3.plot(rolling_mean.index, rolling_mean, linewidth=2, color='red', label='1Y Rolling Mean')
    ...
    for i, instrument in enumerate(top_instruments):
        if instrument in positions_before_idm and instrument in positions_after_idm:
            ...
            if len(ratio) > 0:
                ...
                ax4.plot(sample_ratio.index, sample_ratio,
                         alpha=0.7, label=f'{instrument}', linewidth=1)

    # Overlay actual IDM for comparison
    idm_sample = idm_series.iloc[::20]
    ax4.plot(idm_sample.index, idm_sample,
             color='black', linewidth=3, alpha=0.8, label='Actual IDM')

    ax4.set_title('Position Scaling Factor (Pos After / Pos Before)', fontsize=12, fontweight='bold')
    ...
    ax5.axvline(idm_series.mean(), color='red', linestyle='--', linewidth=2,
                label=f'Mean: {idm_series.mean():.3f}')
    ax5.axvline(idm_series.median(), color='green', linestyle='--', linewidth=2,
    ...
    med_idm = (idm_series >= idm_series.quantile(0.33)) & (idm_series <= idm_series.quantile(0.67))
    high_idm = idm_series > idm_series.quantile(0.67)

    ax7.scatter(idm_series.index[low_idm], idm_series[low_idm],
                c='red', alpha=0.6, s=10, label='Low IDM Period')
    ax7.scatter(idm_series.index[med_idm], idm_series[med_idm],
                c='orange', alpha=0.6, s=10, label='Medium IDM Period')
    ax7.scatter(idm_series.index[high_idm], idm_series[high_idm],
                c='green', alpha=0.6, s=10, label='High IDM Period')
    ...
    try:
        ...
        ax8.set_title('IDM vs Portfolio Returns Correlation', fontsize=12, fontweight='bold')
        ax8.set_ylabel('Rolling Correlation (252d)')
        ax8.axhline(y=0, color='gray', linestyle='--', alpha=0.5)
        ax8.grid(True, alpha=0.3)

    except Exception as e:
        ax8.text(0.5, 0.5, f'Portfolio data not available\n{str(e)[:50]}...',
    ...
    plt.suptitle(f'Actual IDM Analysis - {n_instruments} ETF System',
    ...
    idm_analysis_df = pd.DataFrame({
        ...
        'Active_Instruments': active_instruments.reindex(idm_series.index, method='ffill'),
        'IDM_Regime': pd.Series(['Low' if low_idm[i] else 'Medium' if med_idm[i] else 'High'
                                 for i in idm_series.index], index=idm_series.index)
    })
...
def create_robust_pysystemtrade_system(self):
    ...
    while attempt < max_attempts:
        try:
            print(f"Attempt {attempt + 1}: Testing with {len(working_instruments)} instruments")

            # Filter instrument weights to match working instruments
            filtered_weights = {k: v for k, v in self.config.instrument_weights.items()
            ...
            print(f"✅ Success! IDM extracted with {len(working_instruments)} instruments")
...
def extract_actual_idm_with_robust_handling(self):
    ...
    pst_system, idm_series, working_instruments, problematic_instruments = self.create_robust_pysystemtrade_system()
    ...
    if problematic_instruments:
        print(f"⚠️  Excluded Instruments ({len(problematic_instruments)}): {problematic_instruments}")
...
def analyze_actual_idm_performance_robust(self):
    ...
    idm_series, pst_system, working_instruments, problematic_instruments = self.extract_actual_idm_with_robust_handling()
...
def create_robust_idm_analysis_plots(self, idm_series, instrument_weights,
                                     positions_before_idm, positions_after_idm, pst_system,
                                     working_instruments, problematic_instruments):
    ...
    plt.suptitle(f'Actual IDM Analysis - {n_working_instruments}/{n_total_instruments} ETF System',
                 fontsize=16, fontweight='bold', y=0.98)
    plt.tight_layout()
    plt.subplots_adjust(top=0.94)
    plt.show()
    ...
    try:
        ...
        if performance:
            ...
            if idm_metrics:
                ...
                print(f"   - Theoretical Max: √{len(performance)} = {idm_metrics['theoretical_max']:.3f}")
                print(f"   - Practical Cap: {robert_max:.3f}")
                print(f"   - Recommended: Use {robert_max:.3f} for better risk management")

                if current_idm > 2.5:
                    print(f"⚠️  Your system is using {current_idm:.3f} IDM (too high!)")
                    print(f"💡 Should use {robert_max:.3f} per Robert Carver's methodology")
