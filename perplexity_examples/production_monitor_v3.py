...
class SimpleProductionMonitor:
    ...
    def _check_data_quality(self):
        ...
        stale_percentage = (
            stale_count / total_instruments if total_instruments > 0 else 0
        )
    ...
    def _check_system_performance(self):
        ...
        return {"status": status, "issues": issues, "metrics": performance}
            ...
            for instrument in sample_instruments:
                try:
                    ...
                    if rule_returns is not None and len(rule_returns) > 50:
                        sharpe = (rule_returns.mean() / rule_returns.std()) * (
                            252**0.5
                        )
    ...
    def display_summary(self, health_results):
        ...
        print(
            f"Check Time: {health_results['timestamp'].strftime('%Y-%m-%d %H:%M:%S')}"
        )
        ...
        if dq["status"] != "ERROR":
            print(
                f"  Stale instruments: {dq['stale_instruments']}/{dq['total_instruments']}"
            )
