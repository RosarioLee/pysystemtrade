...
class StaticConfigGenerator:
    ...
    def _find_latest_file(self, pattern):
        ...
        if not files:
            raise FileNotFoundError(f"No files matching {pattern} in {self.analysis_dir}")
    ...
    def categorize_instruments_by_cost(self):
        ...
        for tier_name, instruments in tiers.items():
            if instruments:
                ...
                print(f"  {tier_name:30s}: {len(instruments):3d} instruments "
    ...
    def identify_weight_patterns(self, similarity_threshold=0.02):
        ...
        for inst1 in weights_df.index:
            ...
            # Report
            print(f"Pattern {pattern_id:2d}: {len(pattern_instruments):3d} instruments, "
        ...
        print(f"  Instruments per pattern: {np.mean([p['n_instruments'] for p in patterns.values()]):.1f} average")
    ...
    def generate_forecast_weights_yaml(
        ...
        if use_templates and self.weight_templates:
            ...
            for pattern_id, pattern_info in sorted(self.weight_templates.items()):
                ...
                print(f"  Pattern {pattern_id}: {len(instruments)} instruments with {len(weights)} active rules")
    ...
    def generate_complete_config(
        self,
        base_config_file,
        output_file="results/static_system_config.yaml"
    ):
        """
        ...
        print(f"  ✓ Added static forecast_weights for {len(forecast_weights)} instruments")
    ...
    def validate_static_weights(self):
        ...
        for inst, weights in self.recommended_weights.items():
            ...
            if weights and max(weights.values()) > 0.5:
                ...
                issues.append(f"{inst}: Rule {max_rule} has weight {weights[max_rule]:.3f} (>50%)")
    ...
    def create_comparison_report(
        ...
        with open(output_file, 'w') as f:
            ...
            sr_diff = comparison['Static Weights']['Sharpe Ratio'] - comparison['Dynamic Optimization']['Sharpe Ratio']
            sr_pct = (sr_diff / comparison['Dynamic Optimization']['Sharpe Ratio']) * 100
    ...
    def generate_documentation(self, output_file="results/static_weights_documentation.md"):
        """
        ...
        with open(output_file, 'w', encoding='utf-8') as f:
            ...
            if self.weight_templates:
                ...
                for pattern_id, info in sorted(self.weight_templates.items()):
                    ...
                    f.write(f"- **Example instruments:** {', '.join(info['instruments'][:5])}\n")
                    f.write("\n")
