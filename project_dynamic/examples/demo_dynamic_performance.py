...
def create_working_backtest():
    ...
    try:
        ...
        print(
            f"✅ System created with {len(system.get_instrument_list())} clean instruments"
        )
            ...
            try:
                ...
                if len(prices) > 100:
                    ...
                    if len(vol) > 0:
                        target_vol = 0.16 / len(
                            instruments
                        )  # 16% total, divided by number of instruments
...
def create_comprehensive_equity_curve(portfolio_results):
    ...
    fig.suptitle(
        "Dynamic Optimization System - Complete Backtest Results",
        fontsize=16,
        fontweight="bold",
    )
    ...
    try:
        ...
        # Also save as PDF
        plt.savefig(
            "dynamic_optimization_equity_curve.pdf",
            bbox_inches="tight",
            facecolor="white",
        )
